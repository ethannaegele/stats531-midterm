...
=======
>>>>>>> 2a41bb18d80bbf7d9b1bff5f75d91b01b657dde8

A GARCH model can model showing the volatility of the DAX Stock. The GARCH(1, 1) is shown below.
$$
\begin{aligned}
r_t &= \mu_t + \varepsilon_t ; \varepsilon_t &= \sigma_t e_t ; \sigma_t^2 & = \omega + \alpha ; \varepsilon_{t-1}^2 + \beta \sigma_{t-1}^2
\end{aligned}
$$
<<<<<<< HEAD
...
=======
where $r_t$ are the returns, $\mu_t$ is the mean function, $\varepsilon_t$ is the residual at time $t$, and $e_t \sim N(0, 1)$ for simplicity. If we denote $\mathcal{F}_{t-1}$ as the $\sigma$-algebra (intuitively, information) generated by $\varepsilon_t, \sigma_{t-1}^2$ then we have 
