...
### ARMA and ARIMA
To capture autoregressive and moving average components, we use an autoregressive moving average model (ARMA) and and autoregressive integrated moving average model (ARIMA). The main difference is that the ARIMA model differences through its integrated component, which allows it to handle non stationary data; ARMA generally assumes stationary data. We show results below.
