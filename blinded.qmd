...
Therefore, the DAX Opens has a p value of 0.603955, so, we fail to reject the null, so we may assume it is stationary. On the other hand, the DAX Return has a p value of 0 after running the test. We reject the null and there is evidence to show that the data is stationary. 
