...
#####  Model Diagnosis
We check causality and invertibility in this section. Causality is when the AR process has all roots that lie outside the unit circle. Invertability is when the MA process has all roots that lie outside the unit circle. We diagnose our best solution, ARIMA (3,1,2) below - it is both causal and invertible. 
...
Thus, $r_t^2$ is a conditionally unbiased estimator for $\sigma_t^2$ (and the two are equal in marginal expectation). Thus, it is favorable to use $r_t^2$ as a realized volatility proxy. The disadvantage is that although it is conditionally unbiased, it is high variance. 
